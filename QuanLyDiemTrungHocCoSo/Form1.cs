...
namespace QuanLyDiemTrungHocCoSo
{
    public partial class Form1 : Form
    {
        ...
        private void displayStudents()
        {
            ...
            using (DataTable studentTable = student.objectList("prcStudents", "tblHocSinh"))
            {
                DataView studentView = new DataView(studentTable);
                dgv_mainStudents.AutoGenerateColumns = false;
                dgv_mainStudents.ReadOnly = true;
                dgv_mainStudents.DataSource = studentView;
            }
        }
        private void displayClassSubject()
        {
            string connectionString = ConfigurationManager.ConnectionStrings["MyDatabase"].ConnectionString;
            model.Class myClass = new model.Class();
            using (DataTable classTable = myClass.objectList("procClassSubject", "tblLop_Mon"))
            {
                DataView classView = new DataView(classTable);
                dgv_classSubject.AutoGenerateColumns = false;
                dgv_classSubject.ReadOnly = true;
                dgv_classSubject.DataSource = classView;
            }
        }
        ...
        private void dgv_classSubject_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            LoadSerial(dgv_classSubject);
        }

        private void dgv_mainStudents_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            LoadSerial(dgv_mainStudents);
        }
